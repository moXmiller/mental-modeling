...
def retrieve_images(images_path):
    ...
    if images_path == None: images_path = "/cluster/scratch/millerm/images_mw/trans/mwp_empty.gv.png"
...
def compose_mental_prompt_vlm(sentences, state_idx, new_prompt, lf_dict = {}): #, ic_total = 2):
    ...
    if state_idx == 0:
        ...
        ins = ins_after
        # lf_ins = "This is the JSON representation: '" + lf + "'." ########## change this to "JSON object"?????
...
# def quote_non_numeric(text: str):
#     pattern = r'(?<!["\w])([a-zA-Z_][a-zA-Z0-9_]*)(?!["\w])'
#     text = re.sub(pattern, wrap_non_numeric, text)
#     return text
...
def main(args, tensor_parallel_size = 4, template_file = "data/temp_no_plural.csv"):
    ...
    if step_by_step:
        ...
        # if raw_outputs: csv_field_names = ["problem_idx", "sentence_idx", "prompt", "generated_output"]
        if not raw_outputs: 
