...
# [LABEL] has [NUM] [ENTITY].
# text = "Zsa Zsa has 7 bags."
...
def match_unit_entity(entity_lf, lf, ent_unit_names, entity_to_unit, match, index):
    ...
    if element not in ent_unit_names: return lf
        ...
        return lf
    # else: raise NameError(f"{element} is not listed in the entity-unit dictionary")
...
def check_complete_temp(temp, index):
    ...
    inferred_concepts = []
    # print(f"tmp: {tmp_concepts}")
    # print(f"lf: {lf_concepts}")
...
def pars_ad_totum(part_whole_path = "data/part_whole_entities.json"):
    # with open(part_whole_path) as json_file:
    #     part_whole_entities = json.load(json_file)
...
def replace_wide_entities(concept, lf, match, index, pat_dictionary):
    ...
    if element not in pat_dictionary: return lf
        ...
        # concept = re.search(r"\[([A-Z_]+)\]", concept).group(1)
...
# lf = fill_logical_form(text)
# print(lf)

# def fill_logical_form_wide(text, part_whole_path = "data/part_whole_entities.json", template_file = 'data/temp_part_whole.csv'):
#     temp = load_temp_table(template_file)
#     temp = create_re_temp(temp)
#     parts = [tmp for idx, tmp in enumerate(temp["temp"]) if temp["type"][idx] == "part"]
#     qcons = [tmp for idx, tmp in enumerate(temp["temp"]) if temp["type"][idx] == "q-container"]
#     # print(parts)
#     # print(qcons)
#     combinations = []
#     for part in parts:
#         for qcon in qcons:
#             if ("UNIT" in part) and ("UNIT" in qcon): combinations.append(part + " " + qcon)
#             if ("ATTRIBUTE" in part) and ("ATTRIBUTE" in qcon): combinations.append(part + " " + qcon)
#             elif (("ATTRIBUTE" not in part) and ("ATTRIBUTE" not in qcon) and (("UNIT" not in part) and ("UNIT" not in qcon))):
#                 combinations.append(part + " " + qcon)
#     for comb in combinations:
#         print(comb)
    
# fill_logical_form_wide("hi")

# lf = fill_logical_form(text, part_whole=True)
# print(lf)