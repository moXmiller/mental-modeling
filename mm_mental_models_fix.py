...
# from mm_linear_depth import initialize_vllm, generate_vllm
...
### imported from mm_linear_depth.py
### do not modify this code block

def initialize_vllm(model_name: str):    
...
def generate_vllm(llm, prompts: str, max_tokens=4096):
  ...
  """
  sampling_params = SamplingParams(temperature=0, n=1, max_tokens=max_tokens)#, stop=["\nQ:","\n(Q)","####","###"]) #### reset stop tokens
  outputs = llm.generate(prompts, sampling_params, use_tqdm = True)
  # completions = [output.text for output in outputs[0].outputs][0]
...
def retrieve_context_lfs(ent_unit_path = "data/ent_unit_names.json", template_file = 'data/temp_no_plural.csv'):
    # empty_lf = "container ( [LABEL] , [NUM] , [ENTITY] , [ATTRIBUTE] , [UNIT] )"
    # filled_lf = "container ( John , 7 , cars , None , None )"
    # text = "John has 7 cars."
    temp = load_temp_table(template_file)
    temp = get_container_lfs(temp)
    ...
    temp_id = 0 # choice(list(range(0,len(temp["type"]))))
...
def compose_prompt(text, initial_sentence, lf_list = [], ic_total = 2):
    ...
    # if total_lfs > 1: lf_ins = f"These are the {total_lfs} container representations separated by a semicolon: '" + lf + "'."
...
def compose_question_prompt(question, lf_list: list):
    ...
    # lf_ins = "These are the container representations separated by a semicolon: '" + lf + "'."
...
# lf = ["container ( Anton , 31 , bananas , None , None )", "container ( Constantine , 12 , bananas , None , None )", "container ( John , 2 , bananas , None , None )"]
# output = "container ( Anton , 31 , bananas , None , None ), container ( Anton , 3 , bananas , None , None ), container ( [Anton] , 31 , bananas , None , None ), container ( [LABEL] , 31 , bananas , None , None ), efiwfowenfo, container ( Markus , 31 , bananas , None , None )"
...
# def sentence_by_sentence(output, problem_type, temp, depth, log_form = [], outs_list = [], index = 0):
#     #msp = partial_msp(body, question, index, problem_type, temp, depth)
#         # prompt, ic_containers = compose_prompt(text=s, lf_list = log_form, initial_sentence=initial_sentence)
#         # output = generate_vllm(LLM, prompt)
#     conts = retrieve_containers(output, log_form, ic_containers)
#     if conts != []:
#         log_form = choice(most_common_containers(conts))
#         log_form = [log_form]
#     gt, pred = within_accuracy(log_form, msp, sentence_idx)
#     if pred == gt: print(f"accurate intermediate prediction at problem index {index}, sentence {sentence_idx}")
#     outputs = {"problem_idx": index, "prompt": prompt, "generated_output": output, "prediction": pred, "ground_truth": gt, "accurate": int(pred == gt)}
#     outs_list.append(outputs)
#     # question_prompt = compose_question_prompt(question, log_form)
#     # question_output = generate_vllm(LLM, question_prompt)
#     return question_output, outs_list, question_prompt
...
def compute_mental_accuracy(depth = 6, problem_type = "part", model_name = "llama3-8b", sample_size = 400):
    ...
    for sentence_idx in range(0,depth + 1):
        ...
        # if sentence_idx == 0: raise ArithmeticError
    ...
    print(f"accuracy for {problem_type}: {acc}")
            ...
            # [output.text for output in outputs[0].outputs][0]
        # out, outs_list, q_prompt = sentence_by_sentence(b, questions.iloc[idx][0], llm, problem_type, temp, depth, log_form = [], outs_list = outs_list, index = idx)
        # if isinstance(out, list): pred, gt = extract_floats(str(out[0]), answers, idx)
