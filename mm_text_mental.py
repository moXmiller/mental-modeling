...
from mm_mental_models_fix import retrieve_context_lfs, retrieve_containers, most_common_containers, within_accuracy, complete_temp
...
def body_inference_text(body, sentence_idx: int, logical_forms: list, llm, sampling_params, args):
    ...
    for index, b in enumerate(body):
        ...
        prompt_text = compose_mental_prompt_text(sentences=sentences, state_idx=sentence_idx, lf_dict=logical_forms[index])# (sentences=sentences, state_idx=sentence_idx, new_prompt=new_prompt, lf_dict=logical_forms[index])
...
# def write_json_lfs(lf_lfs, problem_type, model, depth, new_prompt = False, intermediate = False, sentence_idx = 0, text_only = False):
#     lfs_dir = f"data/mm/mental/{model}/vlm_outputs"
#     if not os.path.exists(lfs_dir):
#         os.makedirs(lfs_dir)
#     if intermediate: 
#         if text_only:
#             if new_prompt: lfs_path = f"{lfs_dir}/logical_forms_json_{problem_type}_{depth}_sentence_{sentence_idx}_new_text_only.jsonl"
#             else: lfs_path = f"{lfs_dir}/logical_forms_json_{problem_type}_{depth}_sentence_{sentence_idx}_text_only.jsonl"
#         else:    
...
def main(args, tensor_parallel_size = 4, template_file = "data/temp_no_plural.csv"):
    ...
    stop_words = ["I hope it is correct.",".0.0.0.0.0.0"] # [f"$\\boxed{{{integer}}}$" for integer in range(-200, 201)] + ["I hope it is correct.",".0.0.0.0.0.0"] # ["**Answer:**", "####", "###", "*Answer:*","*Answer*","I hope it is correct."]
    # if model == "llama3.2-90b": stop_words = ["The final answer is: "] # ['"}.', "'}.", "?}"] + [f"{integer}" + r"}" for integer in range(-200, 201)] + [f"$\\boxed{{{integer}}}$" for integer in range(-200, 201)] + ["I hope it is correct.",".0.0.0.0.0.0"]
    ...
    # if step_by_step and (model == "lla<ma3.2-90b"):
    ...
    if step_by_step:
        ...
        for sentence_idx in range(0,depth + 1):
            ...
            #         outputs = {"problem_idx": index, "sentence_idx": sentence_idx, "prompt": prompt, "generated_output": o}
            #         outs_list.append(outputs)
            if sentence_idx >= 0:
            ...
            new_outs_path = write_csv_text_mental(csv_field_names, model, problem_type, depth, outs_list, step_by_step, new_prompt=new_prompt, raw_outputs=True, sentence_idx=sentence_idx)

            new_lfs_path = write_intermediate_json(model, sentence_idx, depth, outs_path=new_outs_path, problem_type = problem_type, size = sample_size, text = True)
                # else:
                ...
                #     elif new_prompt: json_str = out[out.find('{'):]
                    ...
                    #         # extracted_json = quote_non_numeric(extracted_json)
        ...
        # final_lfs_dir = f"data/mm/mental/{model}/vlm_outputs"
        # if not os.path.exists(final_lfs_dir):
        #     os.makedirs(final_lfs_dir)
        # final_lfs_path = f"{final_lfs_dir}/logical_forms_json_{problem_type}_{depth}.jsonl"
        # if new_prompt: final_lfs_path = f"{final_lfs_dir}/logical_forms_json_{problem_type}_{depth}_new.jsonl"
        # with open(final_lfs_path, 'w') as f:
        #     for lf_dict in lf_lfs:
        #         json.dump(lf_dict, f)
        #         f.write('\n')
        # print(f"Created .jsonl for depth {depth} and problem type {problem_type}!")

        # csv_field_names = ["problem_idx", "sentence_idx", "prompt", "generated_output"]
        # # else: csv_field_names = ["problem_idx", "sentence_idx", "prompt", "generated_output", "prediction", "ground_truth", "accurate"]
        # write_csv(csv_field_names, model, problem_type, depth, outs_list, step_by_step, new_prompt=new_prompt, raw_outputs=True)
