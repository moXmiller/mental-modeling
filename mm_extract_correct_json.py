...
from mm_utils import extract_floats
...
# outs_path_test = f"data/mm/mental/{model}/vlm_outputs/outs_sbs_json_trans_{depth}_vision_mental_modeling.csv"
outs_path_test = "data/mm/mental/llama3.2-90b/vlm_outputs/outs_sbs_json_trans_1_new_sentence_1.csv"
# outs_path_test = "data/mm/mental/llama3.2-3b/text_mental_modeling/outs_sbs_json_trans_6_new_sentence_0_text.csv"
# outs_path_test = "data/mm/mental/llama3.2-90b/text_mental_modeling/outs_sbs_json_trans_6_new_sentence_5_text.csv"
...
def write_intermediate_json(model, sentence_idx, depth, outs_path=outs_path_test, problem_type = "trans", size = 400, text = False, image = False):
    ...
    for idx, o in enumerate(generated_outputs):
        ...
        if isinstance(ooo,str): 
            ...
            if "agent" in ooo and "quantity" in ooo and "entity" in ooo and "attribute" in ooo and "unit" in ooo and re.search(r'\s*\d+\s*', ooo):
                # all_dicts = re.findall(r"\{[\S\s]*agent[\S\s]*:[\S\s]*,[\S\s]*quantity[\S\s]*:[\S\s]*\d
                # +[\S\s]*,[\S\s]*entity[\S\s]*attribute[\S\s]*unit[\S\s]*\"\s*\}", ooo)
            ...
            print(f"No logical form at index {idx}")
            #extract_floats(ooo, outs["ground_truth"], idx)
...
if __name__ == "__main__":
    write_intermediate_json(model=model, sentence_idx=sentence_idx, depth=depth, text=text, image=image)